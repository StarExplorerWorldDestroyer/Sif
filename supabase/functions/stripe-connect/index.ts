...
Deno.serve(async (req) => {
  ...
  if (!accountId) {
    ...
    const account = await stripe.accounts.create({
      // Modern Connect: platform controls fees + loss liability, stylist gets an
      // Express dashboard. Stripe collects the required onboarding info.
      controller: {
  ...
  if (action === 'status') {
    ...
    await admin.from('stripe_accounts').update({ ...status, updated_at: new Date().toISOString() }).eq('user_id', uid);
    return json(status);
  }
